...
class SiameseNetwork(nn.Module):
    def __init__(self, net_in):
        ...
        # resnext50_32x4d
        # Pretrained only 23.8%
        # Pretrained and addapter (1kx10 trained) 30.2% <-- Vurderer å prøve å trene den her noe mer
        # Adapted only (1kx10 trained) 11% eller noe.
        self.net_in = net_in
...
params = list(net.children())[:-1]
net = torch.nn.Sequential(*params)
...
val_loader = torch.utils.data.DataLoader(val_set, batch_size=64, num_workers=32, shuffle=True)
...
siameseBaseLine = SiameseNetwork(net).to(cuda_device)  # Net().to(cuda_device)
...
model = SiameseNetwork(net).to(cuda_device)
# model = models.resnet18(pretrained=True).to(cuda_device)
load_checkpoint(model, f'./model_iteration_minimum_tl.pt', optimizer)
